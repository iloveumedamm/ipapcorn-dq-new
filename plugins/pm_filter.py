                ...
                if NO_RESULTS_MSG:
                       await bot.send_message(req_channel,f"-🦋 #REQUESTED_CONTENT 🦋-\n\n📝**Content Name** :`{query}`\n**Requested By**: {query.from_user.first_name}\n **USER ID**: {query.from_user.id}\n\n🗃️",
                                                                                                       reply_markup=InlineKeyboardMarkup([
                                                                                                                                        [InlineKeyboardButton(text=f"🤞Request Recieved", callback_data=f"notify_user_req_rcvd:{reqstr1}:{query}")],
                                                                                                                                        [InlineKeyboardButton(text=f"✅Upload Done", callback_data=f"notify_userupl:{reqstr1}:{query}")],
                                                                                                                                        [InlineKeyboardButton(text=f"⚡Already Upl..", callback_data=f"notify_user_alrupl:{reqstr1}:{query}"),InlineKeyboardButton("🖊Spell Error", callback_data=f"notify_user_spelling_error:{reqstr1}:{query}")],
                                                                                                                                        [InlineKeyboardButton(text=f"😒Not Available", callback_data=f"notify_user_not_avail:{reqstr1}:{query}")],
                                                                                                                                        [InlineKeyboardButton("❌Reject Req", callback_data=f"notify_user_req_rejected:{reqstr1}:{query}")]
                                                                                                           ])
                                             )
